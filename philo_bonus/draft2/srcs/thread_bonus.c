...
/*   Updated: 2024/07/11 20:27:02 by caguillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (pthread_create(&(*phi).thread, NULL, &monitor, phi) != 0)
	{
...
// // // if mut_id = 1 means init[0] failed, the first to be init,
// 	so destroy nothing
// // // if mut_id = nb_philo means init[nb-1] failed, so destroy til init[nb-2]
// // // if mut_id = nb_philo + 1 means init[print] failed,
// 	so destroy til init[nb-1]
// // // if mut_id = nb_philo + 2 means init[dead] failed,
// 	so destroy til init[print]
// // // if mut_id = nb_philo + 3 means init[meal] failed,
// 	so destroy til init[dead]
